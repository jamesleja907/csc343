...
OpenUser(uid, lastSeen) := 
	...
	(User \bowtie_{user.uid = LastSeenAll.uid} LastSeenAll)
$\\\\
...
Q5(\var{uid}, \var{name}, \var{lastSeen}) := 
	...
	(User \bowtie_{user.\var{uid} = MostRecent.\var{uid}} MostRecent) \\ 
$
}
...
\hspace*{1cm} -- Get all direct messages. \\ [5pt]
$
dMessage(mid, from, to, content, time) := \\
\hspace*{1cm} \Pi_{\var{mid}, \var{from}, \var{to}, \var{content}, \var{time}} 
